...
for kv in   MLBB_ONLY_MODE=1 VK_MLBB_DISABLED=1 VK_MLBB_NOTIFY_EMPTY=0 \
  ...
  HIGHLIGHT_MLBB_SKIP_INTRO_SEC=90 MLBB_VOD_FAST_SKIP_INTRO=90 \
  MLBB_VOD_MIN_SEC=180 MLBB_VOD_MAX_SEC=10800 MLBB_VOD_TARGET_DUR_SEC=1800 \
  MLBB_VOD_SKIP_LONG_SEC=10800 MLBB_VOD_MIN_PEAK_SEC=90 \
  ...
  MLBB_EVENT_CLASSIFIER=1 MLBB_EVENT_OWN_MIN_CONF=0.85 MLBB_EVENT_BLOCK_MIN_CONF=0.72 \
  MLBB_EVENT_FAST_BLOCK_MIN_CONF=0.95 MLBB_EVENT_TIER_MIN_CONF=0.62 \
  MLBB_VOD_MONTAGE=0 MLBB_SKIP_MONTAGE=1 \
  ...
  MLBB_VOD_BANNER_HARD_PREFILTER=0 MLBB_VOD_BANNER_PRESEND=1 \
  MLBB_VOD_BANNER_SKIP_ON_MISS=0 MLBB_VOD_MOTION_ANCHOR_OK=0 \
  ...
  MLBB_VOD_MAX_PER_VOD=5 MLBB_VOD_STRICT_PEAK_TRIES=8 MLBB_VOD_SOFT_MAX_PEAK_TRIES=12 \
  MLBB_VOD_SEGMENT_GAP_SEC=45 MLBB_VOD_LENIENT_UNIFORM=1 MLBB_VOD_TAIL_MIN_HUD_RATE=0.40 \
  MLBB_VOD_FEED_STUCK_SEC=10800 \
  ...
  HIGHLIGHT_MAX_PANN_PROBE=16 HIGHLIGHT_MAX_STAGE1=16 HIGHLIGHT_HEATMAP=0 \
...
install -m 755 \
  ...
  "$REPO/scripts/mlbb_event_classifier.py" \
  "$REPO/scripts/mlbb_event_train.py" \
...
export MLBB_VOD_MIN_PEAK_SEC=90
