...
for kv in   MLBB_ONLY_MODE=1 VK_MLBB_DISABLED=1 VK_MLBB_NOTIFY_EMPTY=0 \
  ...
  SHOOTER_VOD_FAST_PROBE=1 SHOOTER_VOD_PREFER_RUSSIAN=1 SHOOTER_VOD_SKIP_INTELLICLIP=0 \
  SHOOTER_VOD_MAX_PANN_PROBE=32 SHOOTER_VOD_SEED_FROM_FAST_PROBE=1 \
  SMART_LONG_SAMPLE_FPS=1.0 SMART_LONG_ANALYSIS_MAX_FPS=1.0 VOD_ANALYSIS_CACHE_REV=2 \
...
install -m 755 \
  ...
  "$REPO/scripts/pubg_owner_calibration.py" \
...
# Prefer .video_bot.env denser PUBG settings; fall back to MLBB-light defaults.
export HIGHLIGHT_MAX_PANN_PROBE="${HIGHLIGHT_MAX_PANN_PROBE:-5}"
export HIGHLIGHT_MAX_STAGE1="${HIGHLIGHT_MAX_STAGE1:-48}"
export SHOOTER_VOD_SKIP_INTELLICLIP="${SHOOTER_VOD_SKIP_INTELLICLIP:-0}"
export SHOOTER_VOD_MAX_PANN_PROBE="${SHOOTER_VOD_MAX_PANN_PROBE:-32}"
export SMART_LONG_SAMPLE_FPS="${SMART_LONG_SAMPLE_FPS:-1.0}"
export SMART_LONG_ANALYSIS_MAX_FPS="${SMART_LONG_ANALYSIS_MAX_FPS:-1.0}"
export HIGHLIGHT_PARALLEL_WORKERS="${HIGHLIGHT_PARALLEL_WORKERS:-6}"
export SMART_FFMPEG_THREADS="${SMART_FFMPEG_THREADS:-6}"
