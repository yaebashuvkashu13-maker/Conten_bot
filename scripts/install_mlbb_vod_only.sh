...
for kv in   MLBB_ONLY_MODE=1 VK_MLBB_DISABLED=1 VK_MLBB_NOTIFY_EMPTY=0 \
  ...
  MLBB_VOD_OWNER_EXEMPLARS=1 MLBB_VOD_SYNC_OWNER=1 MLBB_VOD_MIN_CLIP_SCORE=0.12 \
  MLBB_OWNER_ANCHORS=0 MLBB_SOFT_ANCHOR=0 \
  ...
  MLBB_VOD_BANNER_PRESEND=1 MLBB_VOD_BANNER_SKIP_ON_MISS=0 \
  MLBB_KILL_BANNER_DISCOVER_STEP=4 MLBB_KILL_BANNER_DISCOVER_MAX_PROBES=20 MLBB_KILL_BANNER_DISCOVER_MAX_SEC=120 \
  ...
  PUBG_VOD_KILL_DISCOVER=1 PUBG_KILL_MIN_TIER=single PUBG_VOD_LEAD_SEC=3 \
  PUBG_VOD_VARIABLE_LENGTH=1 PUBG_FIGHT_MIN_SEC=8 PUBG_FIGHT_MAX_SEC=45 PUBG_FIGHT_HARD_MAX_SEC=55 \
  PUBG_KILL_DISCOVER_MAX_PROBES=16 PUBG_KILL_DISCOVER_MAX_SEC=90 \
...
# EU unified: MLBB + PUBG + Standoff on one VPS (server 2).
if [[ -f /root/data/mlbb/EU_UNIFIED_SERVER ]]; then
  rm -f /root/data/mlbb/EU_PUBG_ONLY 2>/dev/null || true
  sed -i 's/^DAILY_MLBB_QUOTA=.*/DAILY_MLBB_QUOTA=10/' "$ENV_FILE"
  sed -i 's/^DAILY_PUBG_QUOTA=.*/DAILY_PUBG_QUOTA=10/' "$ENV_FILE"
  sed -i 's/^DAILY_STANDOFF_QUOTA=.*/DAILY_STANDOFF_QUOTA=10/' "$ENV_FILE"
  sed -i 's/^SHOOTER_VOD_DISABLED=.*/SHOOTER_VOD_DISABLED=0/' "$ENV_FILE"
  grep -q '^SHOOTER_VOD_DISABLED=' "$ENV_FILE" 2>/dev/null || echo 'SHOOTER_VOD_DISABLED=0' >>"$ENV_FILE"
...
elif [[ -f /root/data/mlbb/EU_PUBG_ONLY ]]; then
...
PUBG_SEARCH_CSV="$(python3 - <<PY
import sys
sys.path.insert(0, "$REPO/scripts")
from youtube_shooter_vod_prefs import default_pubg_vod_search_queries_csv
print(default_pubg_vod_search_queries_csv())
PY
)"
sed -i '/^MLBB_VOD_SEARCH_QUERIES=/d' "$ENV_FILE"
printf 'MLBB_VOD_SEARCH_QUERIES="%s"\n' "$VOD_SEARCH_CSV" >>"$ENV_FILE"
sed -i '/^PUBG_VOD_SEARCH_QUERIES=/d' "$ENV_FILE"
printf 'PUBG_VOD_SEARCH_QUERIES="%s"\n' "$PUBG_SEARCH_CSV" >>"$ENV_FILE"
...
install -m 755 \
  ...
  "$REPO/scripts/pubg_dislike_reasons.py" \
  "$REPO/scripts/pubg_kill_banner.py" \
  "$REPO/scripts/pubg_learning_audit.py" \
...
export MLBB_OWNER_ANCHORS=0
export MLBB_SOFT_ANCHOR=0
...
export MLBB_VOD_SEGMENT_GAP_SEC=120
...
export MLBB_VOD_MIN_CLIP_SCORE=0.12
