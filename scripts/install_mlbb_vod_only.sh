...
for kv in   MLBB_ONLY_MODE=1 VK_MLBB_DISABLED=1 VK_MLBB_NOTIFY_EMPTY=0 \
  ...
  SHOOTER_VOD_SOFT_MAX_PEAK_TRIES=6 SHOOTER_VOD_STRICT_PEAK_TRIES=3 \
  SHOOTER_VOD_FAST_MIN_HITS=1 SHOOTER_VOD_FAST_STRONG_PANN=0.40 SHOOTER_VOD_FAST_WEAK_PASS_MIN=0.18 SHOOTER_VOD_FAST_PANN_MIN=0.14 \
  SHOOTER_VOD_FAST_PROBE_MAX=8 SHOOTER_VOD_SCORE_MAX=12 SHOOTER_VOD_ACTION_PEAK_LIMIT=24 \
  SHOOTER_VOD_EXEMPLAR_MAX=12 SHOOTER_VOD_MAX_PANN_PROBE=24 SHOOTER_VOD_SEED_FROM_FAST_PROBE=1 STRICT_PROBE_LIMIT=16 \
  ...
  MLBB_VOD_FEED_STUCK_SEC=1200 \
  SMART_FFMPEG_THREADS=4 \
...
install -m 755 \
  ...
  "$REPO/scripts/pubg_vod_diagnose.py" \
  "$REPO/scripts/shooter_vod_bot_audit.py" \
  "$REPO/scripts/reset_pubg_inbox_exhausted.py" \
