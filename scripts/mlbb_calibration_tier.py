...
TIER_OVERRIDES: dict[int, dict[str, str]] = {
    ...
    2: {
        ...
        "MLBB_SHORTS_MIN_KILL_SCORE": "0.12",
        "MLBB_SHORTS_REQUIRE_KILL_UI": "1",
        "MLBB_KILL_UI_MIN_SCORE": "0.10",
        "MLBB_KILL_ANNOUNCE_SPIKE_MIN": "0.055",
        "MLBB_ACTIVITY_MIN_MOTION": "0.014",
        "MLBB_ACTIVITY_MIN_HUD_DELTA": "0.003",
...
def resolve_tier(*, pending: int, state: dict | None = None, now: float | None = None) -> int:
    ...
    step_sec = float(os.environ.get("MLBB_TIER_STEP_SEC", "360"))  # ~6 min
    starve_sec = float(os.environ.get("MLBB_TIER_STARVE_SEC", "420"))  # ~7 min
...
def note_ingest_saved(*, count: int = 1) -> None:
    """Reset starvation clock after a successful save."""
    if count <= 0:
        return
    state = _load_state()
    now = time.time()
    ...
    state["current_tier"] = min(int(state.get("current_tier", 1)), 1)
    _save_state(state)
