...
Discovery scores a short peak (~10–15s). This module finds the contiguous
hit-flash / impact run that contains the peak and keeps fighting past the
scored peak when flashes continue — same practice as genshin_boss_fight.

Env:
  WOT_BRAWL_FULL_FIGHT=1 / SHOOTER_VOD_VARIABLE_LENGTH=1
  WOT_BRAWL_FIGHT_MIN_SEC=18
  WOT_BRAWL_FIGHT_MAX_SEC=42
  WOT_BRAWL_FIGHT_HARD_MAX_SEC=60
  WOT_VOD_LEAD_SEC=8
  WOT_BRAWL_PEAK_TAIL_SEC=20
  WOT_BRAWL_FIGHT_POST_SEC=2
  WOT_BRAWL_EXPAND_MODE=peak|start
"""
...
def _max_sec() -> float:
    return float(os.environ.get("WOT_BRAWL_FIGHT_MAX_SEC", "42"))


def _hard_max_sec() -> float:
    return float(os.environ.get("WOT_BRAWL_FIGHT_HARD_MAX_SEC", "60"))
...
def detect_brawl_bounds(vod: Path, peak: float) -> tuple[float, float, float]:
    """Return (start, end, dur) for brawl containing peak."""
    import logging
    ...
    step = _step_sec()
    tail = float(os.environ.get("WOT_BRAWL_PEAK_TAIL_SEC", "20"))
    post = float(os.environ.get("WOT_BRAWL_FIGHT_POST_SEC", "2.0"))

    pad = float(os.environ.get("WOT_BRAWL_SEARCH_PAD_SEC", "50"))
    pad = max(hard_max * 0.7, min(pad, 70.0))
    t0 = max(0.0, peak - pad)
    ...
    if run is None:
        ...
        end = min(file_dur, max(peak + tail, start + min_d))
    ...
    if not prefer_start:
        # Peak-centered lead-in, but keep fighting while the flash run continues.
        start = max(0.0, peak - lead)
        end = min(file_dur, max(peak + tail, run_end + post))
        if end - start < min_d:
            end = min(file_dur, start + min_d)
        if end - start > cap:
            # Prefer keeping the finish of the exchange over early setup.
            start = max(0.0, end - cap)
            if peak < start:
                start = max(0.0, peak - lead)
                end = min(file_dur, start + cap)
                end = max(end, min(file_dur, peak + min(tail, 8.0)))
        dur = end - start
    else:
        start = max(0.0, run_start - lead)
        end = min(file_dur, max(run_end + post, peak + tail))
        dur = end - start
        if dur < min_d:
            end = min(file_dur, start + min_d)
            dur = end - start
        if dur > cap:
            need_end = min(file_dur, max(peak + tail, run_end + post))
...
def expand_clip_to_full_brawl(vod: Path, clip: dict) -> dict:
    """Rewrite clip start/duration to cover the brawl through its finish."""
    ...
    return {
        **clip,
        ...
        "wot_brawl_full": True,
        "wot_brawl_dur": dur,
        "wot_brawl_from_start": _expand_mode() == "start",
    }
