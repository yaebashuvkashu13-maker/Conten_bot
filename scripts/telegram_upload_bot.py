...
def start_research_analysis() -> None:
    ...
    def _run():
        try:
            inbox_files = list(RESEARCH_INBOX_DIR.glob("*.csv")) + list(
                RESEARCH_INBOX_DIR.glob("*.xlsx")
            )
            latest = max(inbox_files, key=lambda p: p.stat().st_mtime) if inbox_files else None
            if latest and picked.exists():
