...
def _lock_path() -> Path:
    return Path(
        os.environ.get(
            "POST_QUOTA_MONTAGE_LOCK",
            str(DATA_ROOT / "post_quota_montage.lock"),
        )
    )


def _try_lock():
    """Non-blocking flock so the feed loop cannot spawn parallel montages."""
    path = _lock_path()
...
def run_once(*, token: str = "", chat_id: str = "", max_games: int = 1) -> dict:
    ...
    lock_fh = _try_lock()
    if lock_fh is None:
        return {"skipped": True, "reason": "locked", "day": day, "pending": pending}

    try:
        # Re-check under lock — another tick may have finished while we waited.
        pending = games_pending_today()
        if not pending:
            return {"skipped": True, "reason": "all_done", "day": day}
        ...
        for game in pending[: max(1, max_games)]:
            ...
            if rep.get("ok"):
                ...
                log.warning(
                    "post-quota montage failed game=%s rc=%s",
                    game,
                    rep.get("rc"),
                )
