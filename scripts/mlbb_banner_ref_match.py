#!/usr/bin/env python3
"""Visual match gameplay banner zone against reference bank (wiki + owner photos)."""
...
def _prep_ref_patch(img) -> object | None:
    ...
    # Full screenshots / wiki square → crop banner zone then resize.
    h, w = img.shape[:2]
...
def patch_similarity(patch_a, patch_b) -> float:
    """Hue-robust histogram correlation."""
    import cv2
    ...
    if not rows and root.exists():
        for path in sorted(root.rglob("*.png")):
            if "manifest" in path.name:
                continue
            tier = path.parent.name if path.parent.name in {
                "double", "triple", "maniac", "savage", "unknown", "wiki"
            } else "unknown"
            rows.append((str(path), path.stem, "glob", tier))

    # Owner-confirmed kill screenshots — strongest visual prior.
    photos = owner_photo_root()
    if photos.exists() and os.environ.get("MLBB_BANNER_OWNER_REFS", "1") == "1":
        for path in sorted(photos.glob("*.jpg"))[:80]:
            rows.append((str(path), path.stem[:24], "owner", "double"))
        for path in sorted(photos.glob("*.png"))[:40]:
            rows.append((str(path), path.stem[:24], "owner", "double"))
    return tuple(rows)
...
def match_banner_reference(frame) -> tuple[float, str, str, int] | None:
    ...
    for path, name, source, tier_hint in rows:
        ...
        need = _owner_min_sim() if source == "owner" else _ref_min_sim()
        if score < need:
            continue
...
def classify_banner_reference(sec: float, frame) -> KillBannerHit | None:
    ...
    if tier < _min_tier():
        # Owner/wiki matches without OCR still count as double-tier announce.
        if source in ("owner", "wiki", "glob") and tier < 2:
            tier = 2
        if tier < _min_tier():
            return None
