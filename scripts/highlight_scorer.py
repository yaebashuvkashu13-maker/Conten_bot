...
def score_killfeed_ocr(video_path: Path, start_sec: float, duration_sec: float) -> tuple[str, int]:
    profile = normalize_profile(os.environ.get("_HIGHLIGHT_PROFILE", ""))
    if profile == "mobile_legends":
        try:
            from mlbb_kill_ui import score_mlbb_kill_ui

            result = score_mlbb_kill_ui(video_path, start_sec, duration_sec, sample_frames=6)
            return result.ocr_snippet, result.keyword_hits
        except ImportError:
            pass
    try:
    ...
    *,
    ...
    if profile == "mobile_legends":
        ...
        if metrics.clip_score <= 0.03:
            return False, f"clip_low={metrics.clip_score:.3f}"
        if video_path is not None and os.environ.get("MLBB_REQUIRE_KILL_UI", "1") == "1":
            try:
                from mlbb_kill_ui import passes_mlbb_kill_gate

                ok_kill, kill_reason, kill = passes_mlbb_kill_gate(
                    video_path,
                    start_sec,
                    duration_sec,
                ...
                if not ok_kill:
                    return False, f"no_kill_ui={kill_reason}:score{kill.score:.3f}"
...
def stage1_candidates(video_path: Path, profile: str) -> list[float]:
    ...
    if not owner_anchors_enabled() and profile in ("mobile_legends", "genshin", "wot"):
        ...
        action_peaks = _action_peak_starts(analysis, profile, limit=peak_limit)
        for peak_start in action_peaks:
    ...
    if profile == "mobile_legends" and os.environ.get("MLBB_KILL_GUIDED_SCAN", "1") == "1":
        try:
            from mlbb_kill_ui import filter_starts_by_kill_ui

            seed = sorted({round(float(s), 1) for s in action_peaks}) or sorted(starts)
            kill_starts = filter_starts_by_kill_ui(video_path, seed)
            for s in kill_starts:
                if s >= _mlbb_skip_intro_sec():
                    starts.add(round(max(0.0, s - WINDOW_SEC * 0.35), 1))
            log.info(
                "highlight kill-ui filter %s: %s/%s windows",
                video_path.name,
                len(kill_starts),
                len(seed),
            )
        except Exception as exc:
            log.warning("kill-ui filter failed %s: %s", video_path.name, exc)
