#!/usr/bin/env python3
"""Dense 1 Hz banner audit on inbox / title-queue VODs — no frame skips."""
...
def audit_vod(vod: Path, *, min_tier: int | None = None) -> dict:
    dur = _ffprobe_duration(vod)
    blob = vod_title_blob(vod)
    title_tier = title_min_banner_tier(blob)
    need = min_tier if min_tier is not None else (title_tier if title_tier > 0 else None)
    os.environ["MLBB_VOD_BANNER_DENSE_SEC"] = "1"
    os.environ["MLBB_VOD_BANNER_DISCOVER"] = "1"
    os.environ["MLBB_KILL_BANNER_DISCOVER_MAX_PROBES"] = str(max(600, int(dur) + 64))
    os.environ["MLBB_KILL_BANNER_DISCOVER_MAX_SEC"] = str(max(900.0, min(2400.0, dur * 1.5)))
    if title_tier > 0:
...
def main() -> int:
    ...
    ap.add_argument("--vod", type=Path, action="append", default=[])
    ...
    vods = list(args.vod)
    if not vods:
        vods = _largest_vods(args.inbox or _inbox_root(), args.limit)

    results = []
    for vod in vods:
        if not vod.is_file():
            print(f"skip missing {vod}", file=sys.stderr)
            continue
        print(f"audit {vod.name} …", flush=True)
        row = audit_vod(vod, min_tier=args.min_tier)
        results.append(row)
        print(
            f"  hits={row['hits']} by_tier={row['by_tier']} "
            f"title_min={row['title_min_tier']} scan={row['scan_sec']}s",
            flush=True,
        )
