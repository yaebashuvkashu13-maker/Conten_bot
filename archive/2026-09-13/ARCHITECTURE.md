...
| `scripts/mlbb_vod_segment_feed.py` | MLBB-oriented feed helpers (shared patterns) |
| `scripts/daily_game_cycle.py` | multi-game daily quotas (often disabled in PUBG-only) |
...
| `scripts/dislike_reason_gates.py` | map owner 👎 reasons → hard rejects |
...
```
/root/data/pubg/pubg_owner_labels.json      # owner 👍/👎 by VOD/peak
/root/data/pubg/pubg_moment_ranker.joblib   # champion ranker
/root/data/pubg/vod_segment_labels.json     # segment label bank
/root/data/pubg/vod_segment_state.json      # peak pool / scan state
/root/data/pubg/shorts_autolearn/           # silver features + ranges
/root/data/vod_quality_ledger/pubg_clip_ledger.jsonl
/root/data/vod_adaptive_thresholds/pubg_sent_ranges.json
/root/data/mlbb/*                           # MLBB historical learning
```

## Deploy note

Production often runs **copies** in `/usr/local/bin`. After pull, sync:

```bash
cp -a /root/content_bot_ml/scripts/*.py /usr/local/bin/
# or project install script if present
systemctl restart content-bot-vod-feed.service telegram-upload-bot.service
```

At archive time, only `youtube_download.py` in `/usr/local/bin` was newer than repo; it was copied into git before pause.
