...
6. **Telegram delivery** — `telegram_delivery.py` / `telegram_upload_bot.py` → bot **programofloyalbot** to owner Anton. Inline ratings (+ reasons).
...
| Runtime mirrors | `/usr/local/bin/*.py` (PYTHONPATH=`/root/content_bot_ml/scripts:/usr/local/bin`) |
...
## Commits preserved in this pause window
...
|---------|---------|----------------|
| Disk full | Downloads fail, feed stalls | `vps_disk_cleanup.sh` + timer |
| Reject drought | Many presend rejects, zero sends | Hang detector silence classify + backoff |
| Hang thrash | Feed stuck on one VOD | `vod_hang_detector.py` tick; pin/unpin; heartbeats |
| Long VODs | Slow scans, timeouts | Stage timeouts; ffmpeg timeout scoped to encode |
| Early-cut fights | Owner dislike ended early | Longer post-kill; fight cluster extend while gun hot |
| Loot / run false kills | Silent low gun | `da82a6b` low-gun loot gate |
| Branch divergence | Local vs origin probe8 | Kept prod short defaults (pre=0.8, single_max=18) |
...
Follow `RESTORE.md`. Learning pack under `archive/2026-09-13/data/` + `learning_compressed/` + `tarball_parts/`.
