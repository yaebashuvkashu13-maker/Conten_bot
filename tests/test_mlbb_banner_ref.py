...
def test_patch_similarity_identical() -> None:
    # Need structure — flat color makes edge NCC undefined → hist-only floor.
    patch = np.zeros((48, 160, 3), dtype=np.uint8)
    patch[:, :, 0] = 200
    patch[:, :, 1] = 120
    patch[4:10, :] = (255, 255, 255)
    patch[20:28, 40:120] = (30, 30, 30)
    patch[:, 2:5] = (255, 255, 255)
    assert patch_similarity(patch, patch) > 0.95
