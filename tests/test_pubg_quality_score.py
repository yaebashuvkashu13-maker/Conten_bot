...
def test_low_conf_hud_fp_strong_panns_survives_unproven_but_needs_flash(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """hud_fp + strong PANNs survives unproven; without real flash, not an author kill.

    Wg9@670 fog loot had PANNs≈0.76 and UI glare flash≈0.019 — must not ship.
    """
    monkeypatch.setenv("PUBG_HARD_REJECT_MENU_OVERLAY", "0")
    monkeypatch.setenv("PUBG_EARLY_PAYOFF_REJECT", "0")
    monkeypatch.setenv("PUBG_REJECT_MENU_LOOT_UI", "0")
    ...
    patches[4] = patch(
        ...
        return_value=(
            ...
            "ok",
            {
                "best_hit_flash": 0.019,
                "best_weapon_edge": 0.12,
                "frames": [
    ...
    assert report.get("author_kill_cleared_hud_fp_no_flash") is True
    assert report.get("kill_notification_hit") is False
    assert (report.get("author") or {}).get("has_author_kill") is not True
    assert report.get("author_kill_panns_flash") is not True


def test_low_conf_hud_fp_strong_panns_with_flash_keeps_kill(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Real fight: heuristic hud_fp + strong PANNs + strong hit-flash = author kill."""
    monkeypatch.setenv("PUBG_HARD_REJECT_MENU_OVERLAY", "0")
    monkeypatch.setenv("PUBG_EARLY_PAYOFF_REJECT", "0")
    monkeypatch.setenv("PUBG_REJECT_MENU_LOOT_UI", "0")
