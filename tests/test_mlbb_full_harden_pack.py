...
def test_reliable_keep_banner_miss_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # KEEP when pool still has shippable banners; barren VODs still finish.
    ...
    assert float(os.environ["MLBB_BANNER_OWN_HUD_MIN_SIM"]) <= 0.28
    assert float(os.environ["MLBB_BANNER_DISCOVER_OWN_HUD_MIN_SIM"]) <= 0.22
