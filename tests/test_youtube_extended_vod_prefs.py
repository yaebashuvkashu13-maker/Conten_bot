...
def test_wot_rank_prefers_combat_titles() -> None:
    rows = [
        {"title": "WoT Blitz regular match replay", "duration": 600},
        {"title": "WOT 7 KILLS 9.8K DAMAGE ACE TANKER", "duration": 720},
    ]
    ranked = rank_discovery_candidates("wot", rows)
    assert "ACE TANKER" in ranked[0]["title"]


def test_wot_custom_search_queries(monkeypatch) -> None:
    monkeypatch.setenv("WOT_VOD_SEARCH_QUERIES", "custom ace tanker query,custom brawl")
    params = vod_discovery_search_cycle(0, "wot", {})
    assert params["queries"][0] == "custom ace tanker query"
