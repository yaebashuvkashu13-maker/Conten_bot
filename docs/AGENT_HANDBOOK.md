...
Команды бота: `/start`, `/status`, `/process` (кнопка **Процесс**), `/reset` (кнопка **Сброс**), `/clear`, `/make` — **полный набор только у владельца** (`TG_CHAT_ID`).
Кнопки внизу чата после `/start`: **Процесс** — живой статус пайплайна (что ищет, inbox, серия нулей); **Сброс** — снова открыть исчерпанные VOD и начать поиск YouTube с первой пачки запросов.
