...
Команды бота: `/start`, `/status`, `/process` (статус пайплайна текстом), `/reset` (сброс исчерпанных VOD), `/clear`, `/make` — **полный набор только у владельца** (`TG_CHAT_ID`).
Постоянных кнопок нет — только текстовые команды по потребности.
