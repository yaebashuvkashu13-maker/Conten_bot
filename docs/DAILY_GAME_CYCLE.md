...
00:00 MSK ──► сброс счётчиков (Europe/Moscow)
         │
         ▼
    MLBB (5 клипов/день) ──► PUBG (3) ──► Standoff 2 (3) ──► Genshin (5) ──► WoT (3) ──► idle до 00:00 MSK
```

| Этап | Скрипт | Квота (env) |
|------|--------|-------------|
| MLBB | `mlbb_vod_segment_feed.py` | `DAILY_GAME_MLBB_QUOTA=5` |
| PUBG | `shooter_vod_segment_feed.py pubg` | `DAILY_GAME_PUBG_QUOTA=3` |
| Standoff | `shooter_vod_segment_feed.py standoff` | `DAILY_GAME_STANDOFF_QUOTA=3` |
| Genshin | `shooter_vod_segment_feed.py genshin` | `DAILY_GAME_GENSHIN_QUOTA=5` |
| WoT | `shooter_vod_segment_feed.py wot` | `DAILY_GAME_WOT_QUOTA=3` |

Устаревшие ключи `DAILY_MLBB_QUOTA` / `DAILY_PUBG_QUOTA` / … (со значением 10) **не используются** — приоритет у `DAILY_GAME_*`.
